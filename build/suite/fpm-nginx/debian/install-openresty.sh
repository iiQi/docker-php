...
echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/openresty.asc] $RESTY_APT_REPO $(. /etc/os-release && echo "$VERSION_CODENAME") openresty" | tee /etc/apt/sources.list.d/openresty.list
apt-get update
